...
void Uart :: sendUart(unsigned char *msg){
  ...
  for (int i = 0; i < 256; i++) {
    if(msg[i] != '#') {
      ...
      *p_tx_buffer++ = msg[i];
      *p_tx_buffer++ = msg[i+1];  // *p_tx_buffer++ = '\0';
      break;
    }
  }
  ...
  int msg_size = (p_tx_buffer - &tx_buffer[0]);

  if (fid != -1)
  {
    int count = write(fid, &tx_buffer[0], msg_size);		//Filestream, bytes to write, number of bytes to write

    usleep(1000);   // .001 sec delay

    printf("Count = %d\n", count);

    if (count < msg_size)  printf("UART TX error\n");
  }
